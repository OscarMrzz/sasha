...
BEGIN
    ...
    TRUNCATE TABLE
      ...
      public.premios_escuadra,
      public.premio_escuadra,
      public.escuadras,
    ...
    --   auth.users
    -- RESTART IDENTITY CASCADE;
    -- (Comentado: No se puede truncar auth en Supabase Cloud por restricciones de seguridad)
...
BEGIN
  FOR evento_row IN
    SELECT *
    FROM (VALUES
      ('Inst. Patria', '2026-05-13'::date, 'Principal'),
      ('IBCC', '2026-05-13'::date, 'Aguan'),
      ('Ind. de Saba', '2026-05-13'::date, 'Centro'),
      ('Santa Clara', '2026-05-13'::date, 'Centro'),
  ...
  LOOP
    ...
    /* Asignar eventos de hoy a arrays por zona */
    IF evento_row.fecha_evento = '2026-05-13'::date THEN
      CASE evento_row.zona_evento
...
/* ====================================================================== */
    ...
    /* Asignación de equipo evaluador para eventos antiguos (no hoy) - personal original */
