...
-- Incluye al final: revisar_permisos, RLS (sanciones, copas, solicitar_sancion, solicitud_copas,
...
-- Resuelve roles en la federación FECCAH-DEV (misma lógica que datos_prueba.sql).
...
BEGIN
    ...
    FROM public.federaciones
    WHERE "nombreFederacion" = 'FECCAH-DEV'
    LIMIT 1;

    IF id_federacion IS NULL THEN
        RAISE EXCEPTION 'No existe la federación FECCAH-DEV. Ejecuta primero supabase/snippets/datos/datos_prueba.sql.';
    END IF;

    SELECT "idRol" INTO idRolDeveloper FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'developer' LIMIT 1;
    SELECT "idRol" INTO idRolAdmin FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'admin' LIMIT 1;
    SELECT "idRol" INTO idRolAdminTemporal FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'admin temporal' LIMIT 1;
    SELECT "idRol" INTO idRolJurado FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'jurado' LIMIT 1;
    SELECT "idRol" INTO idRolFiscal FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'fiscal' LIMIT 1;
    SELECT "idRol" INTO idRolDirigente FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'dirigente' LIMIT 1;
    SELECT "idRol" INTO idRolLiderbanda FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'lider de banda' LIMIT 1;
    SELECT "idRol" INTO idRolResponsableBandas FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'responsable de bandas' LIMIT 1;
    SELECT "idRol" INTO idRolResponsableRubricas FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'responsable de rubricas' LIMIT 1;
    SELECT "idRol" INTO idRolResponsableUsuarios FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'responsable de usuarios' LIMIT 1;
    SELECT "idRol" INTO idRolResponsableEventos FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'responsable de eventos' LIMIT 1;
    SELECT "idRol" INTO idRolResponsableMesa FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'responsable de mesa' LIMIT 1;
    SELECT "idRol" INTO idRolSecretaria FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'secretaria' LIMIT 1;
    SELECT "idRol" INTO idRolComiteDisciplina FROM public.roles WHERE "idForaneaFederacion" = id_federacion AND "nombreRol" = 'comite de disciplina' LIMIT 1;

    IF idRolDeveloper IS NULL OR idRolAdmin IS NULL THEN
        RAISE EXCEPTION 'Se requieren roles developer y admin en FECCAH-DEV antes de cargar permisos. Ejecuta datos_prueba.sql primero.';
    END IF;
    ...
    WHERE NOT EXISTS (
        ...
        WHERE p."idForaneaRol" = v.id_foranea_rol
          AND p.tabla = v.tabla
...
-- 1) revisar_permisos: si hay varios perfiles, preferir el vinculado a banda
...
CREATE POLICY "leer_confirmador_banda" ON public.checkout
  ...
  USING (
    EXISTS (
      SELECT 1
      FROM public.perfiles pf
      INNER JOIN public.roles r ON r."idRol" = pf."idForaneaRol"
      WHERE pf."idForaneaUser" = auth.uid()
        AND pf."idForaneaBanda" IS NOT NULL
        AND pf."idForaneaBanda" = checkout.id_foranea_banda
        AND r."nombreRol" IN (
          'dirigente',
          'secretaria',
          'lider de banda',
...
WHERE r."nombreRol" = 'comite de disciplina'
  AND NOT EXISTS (
...
WHERE r."nombreRol" IN ('admin', 'admin temporal')
  AND NOT EXISTS (
    SELECT 1 FROM public.permisos p
    WHERE p."idForaneaRol" = r."idRol"
      AND p.tabla = t.tabla
...
/*
INSERT INTO public.permisos ("idPermiso", "created_at", "idForaneaRol", "tabla", "accion")
SELECT gen_random_uuid(), now(), pf."idForaneaRol", t.tabla, t.accion
FROM auth.users u
INNER JOIN public.perfiles pf ON pf."idForaneaUser" = u.id
CROSS JOIN (VALUES ('checkout', 'UPDATE'), ('checkout', 'SELECT')) AS t(tabla, accion)
WHERE u.email = 'tu-email@ejemplo.com'
  AND pf."idForaneaRol" IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.permisos p
    WHERE p."idForaneaRol" = pf."idForaneaRol"
      AND p.tabla = t.tabla AND p.accion = t.accion
  );
*/
...
SELECT
  ...
  p.accion
FROM auth.users u
JOIN public.perfiles pf ON pf."idForaneaUser" = u.id
JOIN public.roles r ON r."idRol" = pf."idForaneaRol"
LEFT JOIN public.permisos p ON p."idForaneaRol" = r."idRol" AND p.tabla = 'checkout'
WHERE u.email = 'tu-email@ejemplo.com';
*/
