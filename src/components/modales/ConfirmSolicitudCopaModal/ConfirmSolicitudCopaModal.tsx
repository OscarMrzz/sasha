...
type ConfirmSolicitudCopaModalProps = {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void | Promise<void>;
  accion: AccionSolicitud;
  nombreBanda?: string | null;
  lugarCopa?: string | null;
  procesando?: boolean;
};

export default function ConfirmSolicitudCopaModal({
  open,
  onClose,
  onConfirm,
  accion,
  ...
  lugarCopa,
  procesando = false,
}: ConfirmSolicitudCopaModalProps) {
      ...
      >
        <div className="flex w-sm flex-col gap-4 rounded-2xl modal-bg p-6">
          ...
          <p className="text-sm text-slate-300">
            {esAprobar ? (
              <>
                ¿Confirmas aprobar la solicitud de copa
                {lugarCopa ? (
                  <>
                    {" "}
                    <span className="font-bold text-white">{lugarCopa}</span>
                  </>
                ) : null}
                {nombreBanda ? (
                  <>
                ...
                ? Se registrará la copa en el evento.
              </>
            ) : (
              <>
