...
type NegarAsistenciaModalProps = {
  open: boolean;
  nombreEvento: string;
  onClose: () => void;
...
const NegarAsistenciaModal = ({
  open,
  nombreEvento,
  onClose,
  onConfirm,
  loading,
}: NegarAsistenciaModalProps) => {
      ...
      >
        <div className="modal-bg rounded-2xl w-sm flex flex-col gap-4 p-6">
          <div className="flex items-center gap-3">
            <ExclamationTriangleIcon className="w-7 h-7 shrink-0 text-amber-400" />
            <h2 className="text-white text-lg font-bold">Negar asistencia</h2>
          </div>
          <p className="text-slate-300 text-sm">
            ¿Seguro que desea negar su asistencia al evento{" "}
          ...
          <div className="flex justify-end gap-3 mt-2">
            ...
            <button
              ...
              className="px-4 py-2 text-white rounded-lg cursor-pointer transition-colors font-semibold disabled:opacity-50 bg-red-600 hover:bg-red-700"
            >
              {loading ? "…" : "Denegar"}
            </button>
          </div>
...
export default NegarAsistenciaModal;
