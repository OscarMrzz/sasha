...
const OverleyModal = ({ open, onClose, children }: OverleyModalProps) => {
  ...
  const dialogNode =
    open && typeof document !== "undefined" ? (
      ...
      >
        <div className="modal-bg rounded-2xl max-h-120  w-xl flex flex-col overflow-hidden">
          <div className="overflow-hidden scrollbar-estetica p-6">{children}</div>
          <div className=" flex justify-end p-4">
            <button type="button" onClick={onClose} className="border-2 px-4 py-2 modal-text cursor-pointer hover:bg-gray-200/20">
              Cerrar
            </button>
          </div>
        </div>
