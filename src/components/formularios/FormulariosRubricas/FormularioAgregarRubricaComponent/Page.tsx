...
} from "@/services/rubricasServices";
...
export default function FormularioAgregarRubricaComponent({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mensaje =
        error instanceof Error ? error.message : "Error al agregar la rúbrica";
    ...
    <div className="p-2 lg:px-8">
      <div className="mx-auto max-w-lg">
        <header className="mb-8 text-center">
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-primario">Nueva rúbrica</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-white">Agregar rúbrica</h2>
          <p className="mt-2 text-sm text-white/55">Define los datos base para evaluar.</p>
        </header>
        ...
        <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-transparent p-6 shadow-[0_24px_48px_-24px_rgba(0,0,0,0.5)] backdrop-blur-sm sm:p-8">
          <form className="space-y-6" onSubmit={handleSubmit} aria-label="formulario para agregar rubrica">
            <div>
