...
const FormularioEditarBandaComponent = ({
  refresacar,
  onClose,
  bandaAEditar,
  urlLogoBanda,
  ...
  const [formData, setFormData] = useState({
    ...
    ciudadBanda: bandaAEditar.ciudadBanda || "",
  ...
  const [previewUrl, setPreviewUrl] = useState<string>(urlLogoBanda);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await bandaServices.current.initPerfil();

      let urlLogoParaDB = bandaAEditar.urlLogoBanda || "";
      if (selectedFile) {
        try {
          urlLogoParaDB = await bandaServices.current.editarLogoBanda(
            selectedFile,
            `${formData.nombreBanda.replace(/\s+/g, "_")}_logo`
          );
        } catch (logoError) {
          const mensaje = manejoErrorLogoBanda(logoError);
      ...
      const mensaje =
        error instanceof BandaServicioError && error.accion === ACCION_BANDA.LOGO
          ? manejoErrorLogoBanda(error)
          : manejoErrorEdicionBanda(error);
      if (openErrorModal) {
        openErrorModal(mensaje);
      } else {
        alert(mensaje);
    ...
    <div className="p-2 lg:px-8">
      <div className="mx-auto max-w-lg">
        <header className="mb-8 text-center">
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-primario">Editar banda</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-white">Actualizar banda</h2>
        ...
        <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-transparent p-6 shadow-[0_24px_48px_-24px_rgba(0,0,0,0.5)] backdrop-blur-sm sm:p-8">
          <form aria-label="formulario para editar banda" className="space-y-6" onSubmit={handleSubmit}>
