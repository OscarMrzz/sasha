...
export function TablaGeneral({
  ...
  return (
    <section
      className={cn(
        "card-row-bg rounded-2xl border border-[var(--vz-border)]",
        className
      )}
    >
      <details className="group">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-4 py-4 md:px-6">
          ...
          <ChevronDown className="h-5 w-5 shrink-0 text-[var(--app-fg-muted)] transition group-open:rotate-180" />
        </summary>
        <div className="border-t border-[var(--vz-border)] px-2 pb-4 pt-2 md:px-4">
          {!filas.length ? (
            <p className="py-6 text-center text-sm text-[var(--app-fg-muted)]">
            ...
            <ul className="flex max-h-[480px] flex-col gap-2 overflow-y-auto pr-1">
              {filas.map((r, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex w-14 shrink-0 flex-col items-center justify-center border-r border-[var(--vz-border)] pr-3">
                      <span
                        className={cn(
                    ...
                    <div className="flex min-w-0 flex-1 flex-col justify-center gap-0.5 py-1">
                      <span className="truncate font-semibold text-[var(--app-fg)]">
                        {r.nombreBanda}
                        {mine && (
                      ...
                      <div className="flex flex-wrap gap-x-3 text-xs text-[var(--app-fg-muted)]">
