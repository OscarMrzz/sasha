...
/** Orden fijo A–Z por nombre; el estado activo/inactivo no altera la posición en la lista. */
...
export default function ControlesPage() {
  ...
  useEffect(() => {
    const inicializar = async () => {
      ...
      rolesServices.current
        .get()
        .then((roles) => {
          setRolesList(ordenarRolesPorNombre(roles));
        })
        .catch((error) => {
  ...
  const cambiarEstadoRol = (rol: rolInterface) => {
    const nuevoEstado = !rol.estadoRol;
