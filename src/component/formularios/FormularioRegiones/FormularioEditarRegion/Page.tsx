...
import { perfilDatosAmpleosInterface, regionesInterface } from "@/interfaces/interfaces";
...
type Props = {
  ...
  onUpdated?: (region: regionesInterface) => void;
...
export default function FormularioEditarRegionComponent({
  refresacar,
  onClose,
  regionAEditar,
  onUpdated,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (actualizada) onUpdated?.(actualizada as regionesInterface);
      openModalExito?.("Región actualizada correctamente");
    } catch (error) {
      console.error("❌ Error al crear la Federacion:", error);
      openErrorModal?.("Error al editar la región");
    } finally {
      setLoading(false);
      await refresacar();
      onClose();
    ...
    <div className="p-2 lg:px-8">
      <div className="mx-auto max-w-lg">
        <header className="mb-8 text-center">
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-primario">Editar región</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-white">Actualizar región</h2>
          <p className="mt-2 text-sm text-white/55">Modifica el nombre y guarda cambios.</p>
        </header>
        ...
        <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-transparent p-6 shadow-[0_24px_48px_-24px_rgba(0,0,0,0.5)] backdrop-blur-sm sm:p-8">
          <form className="space-y-6" onSubmit={handleSubmit} aria-label="formulario para editar region">
            <div>
