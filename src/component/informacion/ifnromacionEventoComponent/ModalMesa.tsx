...
function esResponsableMesa(registro: registroEquipoEvaluadorDatosAmpleosInterface): boolean {
  return registro.perfiles?.roles?.nombreRol === "responsable de mesa";
}

export default function ModalMesa({ open, onClose, evento, onRefresh }: Props) {
  ...
  const responsablesMesa = useMemo(
    () => equipoEvaluadorList.filter(esResponsableMesa),
    [equipoEvaluadorList],
  );
  ...
  const handleEliminar = async () => {
    ...
    try {
      await registroEquipoEvaluadorServices.current.delete(responsableSeleccionado.idRegistroEvaluador);
      await queryClient.invalidateQueries({ queryKey: ["equipoEvaluador", evento.idEvento] });
      await refetch();
      onRefresh?.();
    } catch (error) {
      console.error("❌ Error al eliminar responsable de mesa:", error);
    }
  };
    ...
    <>
      <OverleyModal open={open} onClose={onClose}>
        <div className="flex max-h-[70vh] min-w-[min(100%,28rem)] flex-col text-white">
          <header className="mb-4 border-b border-white/10 pb-4">
            ...
            <h2 className="mt-1 text-xl font-bold">Responsables de mesa</h2>
          ...
          <div className="mb-4 flex justify-end">
            ...
            >
              ...
              Agregar responsable de mesa
            </button>
          </div>

          <div className="flex flex-1 flex-col gap-3 overflow-y-auto scrollbar-estetica">
            {responsablesMesa.length === 0 ? (
              <div className="rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-white/60">
                No hay responsables de mesa asignados a este evento.
              </div>
            ) : (
              responsablesMesa.map((responsable) => (
                <CardRowEquipoEvaluador
                  key={responsable.idRegistroEvaluador}
                  registro={responsable}
                  abrirInformacion={(reg) => {
                    setResponsableSeleccionado(reg);
                    setOpenVer(true);
                  }}
                  abrirEliminar={(reg) => {
                    setResponsableSeleccionado(reg);
      ...
      <OverleyModal open={openVer} onClose={() => setOpenVer(false)}>
        {responsableSeleccionado ? (
          <InformacionRegistroEquipoEvaluadorComponent registroEquipoEvaluador={responsableSeleccionado} />
        ) : null}
      </OverleyModal>
      ...
      <ConfirmDeleteModal
        ...
        nombreElemento={responsableSeleccionado?.perfiles?.nombre ?? "este responsable de mesa"}
