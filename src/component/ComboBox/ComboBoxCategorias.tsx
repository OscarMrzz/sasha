...
export type ComboBoxCategoriasProps = {
...
export function ComboBoxCategorias({
  ...
  const selected = categorias.find((c) => c.idCategoria === value);
  ...
  const handlePick = (idCategoria: string) => {
  ...
  const triggerClasses = cn(
    ...
    !selected?.nombreCategoria && "text-slate-400",
    className,
  );
    ...
    <PopoverPrimitive.Root open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <span className="min-w-0 flex-1 truncate">
          {selected?.nombreCategoria ?? placeholder}
        </span>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((categoria) => (
                <button
                  ...
                  className={cn(
                    itemBase,
                    categoria.idCategoria === value && itemSelected,
                  )}
                  onClick={() => handlePick(categoria.idCategoria)}
                >
