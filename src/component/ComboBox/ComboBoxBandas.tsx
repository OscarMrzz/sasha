...
import type { bandaInterface } from "@/interfaces/interfaces";
import { cn } from "@/lib/utils";
import { ChevronDownIcon } from "@heroicons/react/24/outline";
import { Popover as PopoverPrimitive } from "radix-ui";
import { useMemo, useState } from "react";

export type ComboBoxBandasProps = {
  bandas: bandaInterface[];
  value?: string;
  onChange: (idBanda: string) => void;
  placeholder?: string;
  disabled?: boolean;
  emptyLabel?: string;
  className?: string;
...
export function ComboBoxBandas({
  bandas,
  value,
  onChange,
  placeholder = "Seleccionar banda",
  disabled = false,
  emptyLabel = "No hay bandas disponibles",
  className,
  popoverClassName,
  id,
}: ComboBoxBandasProps) {
  ...
  const selected = bandas.find((b) => b.idBanda === value);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return bandas;
    return bandas.filter(
      (b) =>
  ...
  const handlePick = (idBanda: string) => {
    onChange(idBanda);
    setOpen(false);
    setQuery("");
  };
  ...
  const triggerClasses = cn(
    ...
    !selected?.nombreBanda && "text-slate-400",
    className
  );
      ...
      <PopoverPrimitive.Root open={open} onOpenChange={handleOpenChange} modal={false}>
        ...
        >
          <span className="min-w-0 flex-1 truncate">
            {selected?.nombreBanda ?? placeholder}
          </span>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((banda) => (
                  <button
                    ...
                    className={cn(
                      itemBase,
                      banda.idBanda === value && itemSelected
                    )}
                    onClick={() => handlePick(banda.idBanda)}
                  >
